...
describe('telemetry partition Prisma/PostgreSQL ownership boundary', () => {
  ...
  it('moves PostgreSQL-only child partitions and the PoP registry into wizer_telemetry', () => {
    expect(migration).toContain('CREATE SCHEMA IF NOT EXISTS wizer_telemetry');
    expect(migration).toContain('ALTER TABLE %s SET SCHEMA wizer_telemetry');
    expect(migration).toContain('ALTER TABLE IF EXISTS public.proof_of_play_session_keys');
    expect(migration).toContain('SET SCHEMA wizer_telemetry');
  });

  it('pins trigger/helper lookup to the internal schema and fails mixed layouts', () => {
    expect(migration).toContain('SET search_path = wizer_telemetry, public');
    expect(migration).toContain('telemetry partition isolation failed');
