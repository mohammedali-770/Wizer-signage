...
describe('FleetHealthService', () => {
  it('aggregates versions and recent bounded crash metadata within one company', async () => {
    ...
    const service = new FleetHealthService(prisma as never);
    const result = await service.summary('company-1');
        ...
        findMany: jest.fn().mockResolvedValue([
          {
            ...
            appVersion: '1.0.0',
            lastHeartbeatAt: null,
            capabilities: { deviceCrash: { fingerprint: 'not-a-fingerprint', crashCount: -1 } },
          },
        ]),
      },
    };
