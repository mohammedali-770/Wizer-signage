...
| 4   | Take/verify the pre-migration backup                                                      | **DONE on the production host** 2026-09-02 — dump taken and byte-verified (228387 bytes). The offsite target was a same-volume placeholder until 2026-09-09; preflight now rejects a local-copy `BACKUP_OFFSITE_CMD` outright, so it cannot silently return                                                                                                      |
