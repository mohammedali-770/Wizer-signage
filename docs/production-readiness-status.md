...
| #   | Item                                                                                      | Status                                                                                                                                                                                                                                                                                                                                                                                                               |
| --- | ----------------------------------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| 1   | Publish immutable API/dashboard/maintenance GHCR images for the exact `main` SHA          | **DONE** 2026-09-02 — `3b61952f4dd0` published for all three images. No separate registry credential was needed: `release-images.yml` publishes with `secrets.GITHUB_TOKEN` and `packages: write`, so a repository owner ran it from the Actions UI. What gated it was `captcha_site_key`, a required workflow input baked irreversibly into the dashboard image                                                     |
...
| 4   | Take/verify the pre-migration backup                                                      | **DONE on the production host** 2026-09-02 — dump taken and byte-verified (228387 bytes). The offsite target was a same-volume placeholder until 2026-09-09 and is now a real DigitalOcean Spaces bucket (`fra1`, bucket-scoped read/write key), verified by listing the object back out of the bucket. Preflight also rejects a local-copy `BACKUP_OFFSITE_CMD` outright, so the placeholder cannot silently return |
...
Five of the six host-dependent items completed on 2026-09-02. **Five items remain**: closing the smoke row (6), Android (10-12), and final EN/AR acceptance (13). None is blocked on engineering, and none still waits on the production host or the captcha key pair — those were satisfied at the cutover. Item 6 needs one smoke re-run against the live release; item 10 the Android signing keystore; items 11-12 a physical screen; item 13 an operator pass over the live EN/AR dashboard.

> **Item 1's blocker was previously recorded as "registry write", which was wrong** and would have sent an operator looking for a credential they do not need. The release workflow authenticates to GHCR with the automatic `GITHUB_TOKEN`. The real gate was the captcha site key, and because it is compiled into the dashboard bundle it had to be correct _before_ item 1 ran rather than fixed afterwards — which is what made it the first thing to obtain, not the last.

## Operator prerequisites

These cannot be satisfied from the repository. Host configuration and the captcha key pair were satisfied at the 2026-09-02 cutover; they are recorded here as the standing requirement for every future release, not as open blockers.
...
- **Preflight compares host against image, not against the live server.** It now compares the host against the image being _deployed_ rather than the one it replaces (fixed 2026-09-09, after that ordering blocked the PostgreSQL 17 deploy), and the client major is 17 across the maintenance image, the host and CI, matching the 17.6 production server — but nothing in the pipeline reads the server's own version. `maintenance-runtime.spec.ts` pins the image to CI's Postgres service and preflight pins the host to the image, so the three cannot drift apart from each other; what they cannot catch is all three drifting away from the server. On a server upgrade, move `infra/docker/Dockerfile.maintenance`, the host package and CI's service together. This note is the only thing watching that last gap.
- **An off-box backup is only as good as its destination, and preflight can only see the command.** Until 2026-09-09 `BACKUP_OFFSITE_CMD` was a `cp` into a second directory on the same volume: it satisfied the no-op check, resolved inside the maintenance image, and produced a verified byte count identical to the local dump, so every gate passed while losing the droplet would have lost the database and all snapshots together. Preflight now refuses `cp`, `mv`, `install`, `ln` and `dd` anywhere in the command — `scripts/tests/preflight-offsite-backup.test.sh` pins that, including the compound `mkdir -p ... && cp ...` form production actually ran. What it still cannot tell is whether a _legitimate_ transport is pointed somewhere useful: `rclone copyto` to a bucket in the same failure domain as the database passes, and should not.

- **A bucket-scoped Spaces key needs `no_check_bucket`, and the error when it is missing accuses the wrong thing.** rclone verifies the destination bucket exists before uploading; that check is `ListBuckets`, an account-level operation no bucket-scoped key has, whatever its read/write permission. rclone then assumes the bucket is absent and attempts `CreateBucket`, which is also denied, so the upload fails with `AccessDenied` naming **CreateBucket** — which reads as a broken credential and sends an operator rotating keys that were never the problem. `RCLONE_CONFIG_SPACES_NO_CHECK_BUCKET=true` skips the check. Confirmed on 2026-09-09 with one unchanged key that failed before the setting and succeeded after it.
