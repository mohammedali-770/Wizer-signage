...
> **This describes the system as built.** Every phase in [roadmap.md](./roadmap.md)
> is complete, so the flows below are the ones that run in production — not a plan.
> Where something was considered and not built (a WebSocket gateway, most notably),
> it is called out as such rather than described as if it exists.
...
| --------------------- | -------------------------------------------------------------------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| **Dashboard**         | Next.js 15 (App Router), React 18, TypeScript, Tailwind 3.4, next-intl v4, next-themes | Operator web UI. Bilingual (English LTR / Arabic RTL), light/dark themes. Talks to the API over REST.                                                                                                     |
...
| **Object storage**    | Supabase Storage                                                                       | Content media (images, video), screenshots, scheduled-report files. Bucket via `SUPABASE_STORAGE_BUCKET`. **Not** APKs — those are host files under `APK_DOWNLOAD_DIR` (default `/srv/downloads`).        |
...
| **Android TV player** | Kotlin 2.4, Jetpack Compose, Media3 ExoPlayer, AGP 8                                   | Native leanback app (`com.wizer.signage`). Plays content, pre-downloads media, reports telemetry.                                                                                                         |
...
Player boots (unpaired)
   -> POST /api/device/pairing/start    (device requests a pairing code)
...
Player POLLS GET /api/device/pairing/status?code=... (x-pairing-secret header)
...
Schedule/playlist changes in dashboard
   ...
   -> API enqueues a REFRESH_MANIFEST command; the player collects it on its
      next poll of GET /api/device/commands/pending (~12s), so a change lands
      within roughly that window rather than instantly
...
```
Player --(every 60s: POST /api/device/heartbeat)--> API
...
A screen is marked OFFLINE after 3 missed heartbeats (see monitoring.constants.ts).
```
...
Operator clicks an action (reboot, screenshot, clear cache, reload, identify)
   ...
   -> API authorizes (role + tenant), enqueues a PENDING DeviceCommand row
   -> Device COLLECTS it on its next poll of GET /api/device/commands/pending (~12s)
   <- Device executes and acknowledges; result stored (e.g. Screenshot row + storage URL)
A pending command EXPIRES after 5 minutes (COMMAND_TTL_SECONDS) and is marked
EXPIRED on the next poll. A screen offline longer than that never receives it —
the command is dropped, not held for reconnect.
```
...
Operator triggers an Emergency Broadcast (company/location/screen scope)
   ...
   -> API enqueues a REFRESH_MANIFEST command for every targeted screen
      (dispatchToScreens -> deviceCommand.createMany), online or not
   -> Players collect it on their next poll (~12s) and re-resolve the manifest
...
- **Nothing is sticky, because nothing holds a socket.** Devices poll, so any
  instance can answer any device on any cycle, and commands live in Postgres
  rather than in an instance's memory. This is the main operational payoff of
  choosing polling (§6) and the reason horizontal scaling needs no extra part.
- **`REDIS_URL` is read into config but nothing consumes it**, and no Redis
  client is installed. The slot exists for the deferred WebSocket fan-out
  (roadmap Phase 8); setting it today has no effect.
...
| Language         | TypeScript 5.4+ (strict); Kotlin 2.4 (player)                            |
| Dashboard        | Next.js 15 App Router, React 18, Tailwind 3.4, next-intl v4, next-themes |
| API              | NestJS 11                                                                |
| ORM              | Prisma                                                                   |
| Database         | Supabase Postgres                                                        |
| Object storage   | Supabase Storage                                                         |
| Realtime         | HTTPS polling — no WebSocket gateway (heartbeat 60s, commands ~12s)      |
