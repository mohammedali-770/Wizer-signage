...
| 2   | **`pg_dump` version skew** — the maintenance image's `postgresql-client` major must match the Supabase Postgres major (**17**) or `pg_dump` refuses to run: _"server version 17.6; pg_dump version 16.x"_. | Already pinned to `postgresql17-client` in `infra/docker/Dockerfile.maintenance`. If the server major moves, move that pin, the host package and CI's Postgres service together. |
